...
import common.GenSort;
import common.Unsigned16;
import common.Random16;
...
public class DataGen extends Configured implements Tool {
    ...
    static class RangeInputFormat extends InputFormat<LongWritable, NullWritable> {
        ...
        /**
         * Create the desired number of splits, dividing the number of rows between the
         * mappers.
         */
    ...
    static long getNumberOfRows(JobContext job) {
        return job.getConfiguration().getLong(DataSortConfigKeys.NUM_ROWS.key(), DataSortConfigKeys.DEFAULT_NUM_ROWS);
    }

    static void setNumberOfRows(Job job, long numRows) {
        job.getConfiguration().setLong(DataSortConfigKeys.NUM_ROWS.key(), numRows);
    }
    ...
    public static class SortGenMapper extends Mapper<LongWritable, NullWritable, Text, Text> {
        ...
        private byte[] buffer = new byte[DataInputFormat.KEY_LENGTH + DataInputFormat.VALUE_LENGTH];
        ...
        public void map(LongWritable row, NullWritable ignored, Context context)
            ...
            key.set(buffer, 0, DataInputFormat.KEY_LENGTH);
            value.set(buffer, DataInputFormat.KEY_LENGTH, DataInputFormat.VALUE_LENGTH);
            context.write(key, value);
            crc32.reset();
            crc32.update(buffer, 0, DataInputFormat.KEY_LENGTH + DataInputFormat.VALUE_LENGTH);
    ...
    private static void usage() throws IOException {
        System.err.println("datagen <num rows> <output dir>");
