...
class DataScheduler {
  ...
  public DataScheduler(String splitFilename, String nodeFilename) throws IOException {
    slotsPerHost = 4;
    // get the hosts
  ...
  public DataScheduler(FileSplit[] realSplits, Configuration conf) throws IOException {
   ...
   * schedule. It will move placed splits to front and unplacable splits to the
   * end.
   * 
   * @return a new list of FileSplits that are modified to have the best host as
   *         the only host.
   * @throws IOException
   */
  public List<InputSplit> getNewFileSplits() throws IOException {
    ...
    for (int i = 0; i < splits.length; ++i) {
      if (splits[i].isAssigned) {
        ...
        realSplits[i] = new FileSplit(realSplits[i].getPath(), realSplits[i].getStart(), realSplits[i].getLength(),
            newLocations);
  ...
  public static void main(String[] args) throws IOException {
    DataScheduler problem = new DataScheduler("block-loc.txt", "nodes");
